...
export const useQuizSubmission = () => {
  ...
  const submitAnswer = async (
    ...
    try {
      ...
      const submitResponse = await quizAPI.submitTodayQuizAnswer(
        quizId,
        submitAnswer,
        subscriptionId,
      );
      ...
      if (submitResponse && typeof submitResponse === "object") {
        responseData =
          "data" in submitResponse ? submitResponse.data : submitResponse;
      } else {
      ...
      if ((responseData as any)?.duplicated) {
        ...
        const duplicateResult: AnswerResult = {
          ...
          commentary:
            (responseData as any)?.commentary || displayQuiz.commentary,
        ...
        if (
          ...
          if (feedback.includes("정답:") || feedback.includes("오답:")) {
            ...
            if (feedbackIndex === -1) {
              ...
              const resultPart = feedback
                .substring(0, feedbackIndex)
                .replace(/^(정답:|오답:)\s*/, "")
                .trim();
      ...
      const userQuizAnswerId =
        (responseData as any)?.userQuizAnswerId?.toString() || "";
      ...
      if (displayQuiz.quizType === "MULTIPLE_CHOICE") {
        ...
        try {
          ...
          const result: AnswerResult = {
            ...
            commentary:
              (evaluateData as any)?.commentary || displayQuiz.commentary,
          };
              ...
              setStreamingFeedback((prev) => {
                ...
                if (newText.includes("정답:") || newText.includes("오답:")) {
                  ...
                  if (feedbackIndex === -1) {
                    ...
                    const resultText = newText
                      .replace(/^(정답:|오답:)\s*/, "")
                      .trim();
                    setFeedbackResult(resultText);
                  } else {
                    // 피드백 부분이 나옴 - 결과와 피드백 분리
                    const resultPart = newText
                      .substring(0, feedbackIndex)
                      .replace(/^(정답:|오답:)\s*/, "")
                      .trim();
                    const feedbackPart = newText
                      .substring(feedbackIndex + 3)
                      .trim(); // '피드백:' 이후
