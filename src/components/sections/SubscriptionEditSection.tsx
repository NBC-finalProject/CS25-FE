...
const SubscriptionEditSection: React.FC = () => {
        ...
        <div className="space-y-2">
          {errors.map((error, index) => (
            <div key={index} className="flex items-start space-x-2 text-left">
              <div className="mt-2 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-red-400"></div>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.period && !validateSubscriptionPeriod(formData.period)) {
      errors.period =
        "구독 기간 연장 시 시작일로부터 1년을 초과할 수 없습니다.";
    }
      ...
      <Section className="bg-gray-50 py-20">
        <Container>
          <div className="text-center">
            ...
            <button
              onClick={() => navigate("/")}
              className="bg-brand-500 hover:bg-brand-600 mt-4 rounded-lg px-4 py-2 text-white transition-colors"
            >
              홈으로 돌아가기
            </button>
          </div>
            ...
            <div className="mb-6 rounded-lg bg-blue-50 p-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`text-sm font-medium ${subscriptionData?.active ? "text-green-600" : "text-red-600"}`}
                  >
                ...
                {categoriesLoading ? (
                  ...
                  <div className="flex space-x-3">
                    {categories.map((category) => (
                      <label
                        key={category.id}
                        className={`flex flex-1 cursor-pointer items-center justify-center space-x-2 rounded-lg border p-3 transition-all duration-300 ${
              ...
              <div>
                ...
                <div className="mb-3 rounded-lg border border-blue-200 bg-blue-50 p-3">
                  <p className="text-sm text-blue-700">
                    ...
                    선택사항: 선택한 기간은 현재 구독 종료일(
                    {subscriptionData?.endDate})로부터 추가됩니다.
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  {periods.map((period) => (
                    <button
                      ...
                      className={`flex items-center justify-center space-x-2 rounded-lg border p-3 transition-all duration-300 ${
              ...
              <div className="flex space-x-4 pt-4">
                ...
                <button
                  ...
                  disabled={
                    ...
                    (!!formData.period &&
                      !validateSubscriptionPeriod(formData.period))
                  }
                  className="from-brand-500 to-brand-600 hover:from-brand-600 hover:to-brand-700 flex-1 rounded-lg bg-gradient-to-r px-4 py-3 font-bold text-white transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-50"
                >
